<template>
    <div>
        <template v-if="!lists.length">
            <p class="text-center">{{ __('Fetching MailChimp lists') }}...</p>
        </template>
        ...
        <template v-else>
            <table class="form-table">
                <tbody>
                    <tr>
                        <th>{{ __('Select MailChimp list') }}</th>
                        <td>
                            <ul>
                                <li v-for="list in lists" class="margin-bottom-10">
                                    <label>
                                        <input type="radio" :value="list.id" v-model="settings.mailchimp_list_id"> {{ list.name }} <small class="text-muted">({{ list.count }})</small>
                                    </label>
                                </li>
                            </ul>
                        </td>
                    ...
                    <tr>
                        <th>{{ __('Import MailChimp list') }}</th>
                        <td>
                            <ul>
                                <li class="margin-bottom-10">
                                    <label>
                                        <input type="radio" :value="true" v-model="settings.import_mailchimp_list"> {{ __('Import subscribers with MailChimp list') }}
                                    </label>
                                </li>
                                <li>
                                    <label>
                                        <input type="radio" :value="false" v-model="settings.import_mailchimp_list"> {{ __('Import subscribers into weMail list') }}
                                    </label>
                                </li>
                            </ul>
                        </td>
                    ...
                    <tr v-if="!settings.import_mailchimp_list">
...
<script>
    export default {
        ...
        created() {
            ...
            weMail.api.import().mailchimp().lists().get().done((response) => {
        ...
        methods: {
            goToNext() {
                if (!this.settings.mailchimp_list_id) {
                    return this.error(__('You must select a MailChimp list'));
                }

...
class WpUser {
    ...
    public function wemail_registration_save( $user_id ) {
        ...
        $access_token = get_user_meta( $user_id, 'wemail_api_key', false );
        if ( $access_token ) {
            wemail()->api->set_api_key( $access_token[0] );
            $roles = $user->roles;
            if ( count( $roles ) > 0 && in_array( 'administrator', array_values( $roles ), true ) ) {
            ...
            if ( count( $roles ) > 0 && ! in_array( 'administrator', array_values( $roles ), true ) && in_array( 'editor', array_values( $roles ), true ) ) {
                $data['role'] = 'team';
                $wp_admin_response = wemail()->api->auth()->users()->post( $data );
                if ( isset( $wp_admin_response['access_token'] ) && $wp_admin_response['access_token'] !== '' ) {
                    update_user_meta( $user_id, 'wemail_api_key', $wp_admin_response['access_token'] );
    ...
    public function wemail_user_deleted( $user_id ) {
        ...
        if ( $access_token ) {
            ...
            if ( $user ) {
                ...
                $wp_admin_response = wemail()->api->auth()->users()->destroy()->post( $data );
